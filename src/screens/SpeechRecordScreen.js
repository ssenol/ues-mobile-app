...
const SpeechRecordScreen = ({ route, navigation }) => {
  ...
  const handleSubmit = async () => {
    try {
      ...
      // Değerlendirme isteği gönder
      console.log('1.) /question/evaluate-speech-pronunciation API isteği gönderiliyor...');
      const evaluationResponse = await speechService.evaluateSpeech(formData);
      console.log('1. Yanıt:', evaluationResponse);

      if (evaluationResponse?.status_code !== 200) {
        throw new Error('Speech evaluation failed');
      }

      // Birden fazla sonuç olması durumunda ortalama hesaplayan yardımcı fonksiyon
      const calculateAverage = (array, propertyPath) => {
        if (!array || array.length === 0) return 0;
        const sum = array.reduce((acc, item) => {
          // propertyPath örn: "NBest.0.PronunciationAssessment.AccuracyScore"
      ...
      // Belirli bir değeri sonuçlardan alır, eğer sonuç yoksa varsayılan değeri döndürür
      const getValueFromResults = (results, propertyPath, defaultValue = '') => {
        ...
        return propertyPath.split('.').reduce((obj, key) => 
          obj && obj[key] !== undefined ? obj[key] : defaultValue, results[0]);
      };
      
      const results = evaluationResponse.data.results || [];
      
      // Tüm sonuçların ortalama süresini hesapla
      const recordingDurationSeconds = Math.round(calculateAverage(results, 'Duration') / 1000000);
      console.log('Kayıt süresi:', calculateAverage(results, 'Duration'));
      console.log('Kayıt süresi (saniye):', recordingDurationSeconds);
      ...
      // Tüm sonuçlardan metin elde et (eğer birden fazla varsa ilkini kullan)
      const displayText = getValueFromResults(results, 'DisplayText', '');
      
      // Konu puanını almak için API isteği gönder
      console.log('2.) /quiz/get-speech-question-topic-related-score API isteği gönderiliyor...');
      const topicScoreResponse = await speechService.getSpeechQuestionTopicRelatedScore({
        subject: speechData,
      ...
      console.log('2. Yanıt:', topicScoreResponse);

      if (topicScoreResponse?.status_code !== 200) {
        throw new Error('Topic score evaluation failed');
      }

      // AI değerlendirmesi için API isteği gönder
      console.log('3.) /question/get-speech-assessment-result-evaluation-by-ai API isteği gönderiliyor...');
      const aiResponse = await speechService.getSpeechAssessmentResultEvaluationByAI({
        speechData,
        taskDetails,
      ...
      console.log('3. Yanıt:', aiResponse);
